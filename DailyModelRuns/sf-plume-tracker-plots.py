...
def generate_animation_img_stack(o, start_date, add_current_vectors=False):
    ...
    if add_current_vectors:
        ...
        #print('shape of hfr_currents')
        #print(hfr_current_vectors.shape)
        
    lons = o.history['lon']
    # code appears to stop early due to all particles leaving or grounding?
    # I'm going to add a piece to the code to only output what exist
    # 
    #******
    lenlon=len(lons)
    #print('longitudes '+str(len(lons)))
    lats = o.history['lat']
    cmap = cmocean.cm.haline
    xx,yy,elv = load_bathy_data()
    gdf = load_roi_shapefiles()

    for hours in tqdm.tqdm(range(lenlon)):
    ...
    os.system(cmd)
    #print(cmd)
    ocmd=f"scp -i /etc/ssh/keys/pdaniel/scp_rsa /home/pdaniel/SurfaceCurrentMaps/DailyModelRuns/model_output/animations/{start_date.strftime('%Y%m%dT%H%M%S')}_48_hour.gif skyrocket8.mbari.org:/var/www/html/data/hfr-particle-tracking-sfbay/sf-plume-tracks.gif"
    #print(ocmd)
    os.system(ocmd)
    pcmd=f"scp -i /etc/ssh/keys/pdaniel/scp_rsa /home/pdaniel/SurfaceCurrentMaps/DailyModelRuns/model_output/animations/{start_date.strftime('%Y%m%dT%H%M%S')}_48_hour_vector.gif skyrocket8.mbari.org:/var/www/html/data/hfr-particle-tracking-sfbay/sf-plume-tracks-vector.gif"
    os.system(pcmd)
